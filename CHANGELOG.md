...
### Список внесенных изменений:
  ...
  1. Переименование  settings.flap  в  settings.curFlap  и добавление  minFlap ,  maxFlap  в структуру настроек:
      • В файле main.h структура  Settings  была изменена:
        uint8_t curFlap;    // Flap current position (0-100%)
        uint8_t minFlap;    // Flap min open (%)
        uint8_t maxFlap;    // Flap max open (%)

      • В файле my_settings.h инициализация настроек по умолчанию изменена:
        .curFlap = 0,
        .minFlap = 0,
        .maxFlap = 100,

      • Обновлены все обращения к переменной в коде прошивки:
          • keypad.cpp — в логике обработки клавиатуры.
          • main.cpp — при инициализации сервопривода и в основном цикле управления.
          • setupLCD.cpp — в меню настройки заслонки на LCD-экране.
          • procedure.cpp — при отладочном выводе в консоль, сбросе к заводским настройкам, а также в функциях
          загрузки и сохранения JSON-конфигурации из LittleFS.

  2. Обновление веб-интерфейса и REST API:
      • В файле setup.html на вкладке «Системні» добавлены новые поля ввода для минимального и максимального
      открытия заслонки:
        <label>Заслінка закрита (%):</label>
        <input type="number" name="minFlap" id="minFlap" min="0" max="100">
        <label>Заслінка відкрита (%):</label>
        <input type="number" name="maxFlap" id="maxFlap" min="0" max="100">

      • В файле server.cpp обновлены обработчики HTTP-запросов:
          •  respondsEeprom  — возвращает значения  minFlap  и  maxFlap  в JSON для отображения на странице настроек.
          •  acceptEeprom  — принимает и сохраняет переданные значения настроек заслонки, а также корректно логирует
          их изменения в системный лог.
          •  respondsProgram  и  programDeser  — обновлены для поддержки  settings.curFlap .

### Список внесенных изменений:

  1. Обновление структуры  TableForOneHour  и объединения  TableBuff  в main.h:
      • Удалены поля  water0run  и  water1run .
      • Добавлены поля  flapMin  и  flapMax .
      • Поле  timerFlap  переименовано в  flapCurr .
      • Поле объединения  spHour  переименовано в  spProg .
  2. Обновление функций в programm.cpp:
      • В функции  prepareTable(...) :
          • Удалено присвоение несуществующих  water0run  и  water1run .
          • Значение  water2run  всегда устанавливается в  0 .
          • Добавлена инициализация заслонки:
              •  unTable.spProg.flapMin = settings.minFlap;
              •  unTable.spProg.flapMax = settings.maxFlap;
              •  unTable.spProg.flapCurr = settings.minFlap;

      • В функции  testProgs()  имя  spHour  заменено на  spProg .

 ### Сравнение распределения памяти AT24C32 (4 КБ / 4096 байт)

   Диапазон адресов (HEX) | Диапазон адресо… | Было (4 программы)                 | Стало (1 программа)
  ------------------------|------------------|------------------------------------|----------------------------------
    0x000  –  0x17F       | 0 – 383          | Программа №1 (24 часа * 16 байт)   | Программа №1 (24 часа * 16 байт)
    0x180  –  0x2FF       | 384 – 767        | Программа №2 (24 часа * 16 байт)   | Не используется (Свободно)
    0x300  –  0x47F       | 768 – 1151       | Программа №3 (24 часа * 16 байт)   | Не используется (Свободно)
    0x480  –  0x5FF       | 1152 – 1535      | Программа №4 (24 часа * 16 байт)   | Не используется (Свободно)
    0x600  –  0x6FF       | 1536 – 1791      | Резерв                             | Не используется (Свободно)
    0x700  –  0xDC3       | 1792 – 3523      | Логи графиков ( DAILY_DATA_START ) | Логи графиков ( DAILY_DATA_START)
    0xDC4  –  0xFFF       | 3524 – 4095      | Резерв / Системные данные          | Резерв / Системные данные

  ### Внутреннее изменение структуры часа ( TableForOneHour ):

  Размер записи за один час по-прежнему составляет 12 байт (в памяти занимает 16 байт с учетом выравнивания):

  • Было: 4 уставки температуры ( spT0on ,  spT0off ,  spT1on ,  spT1off  по 2 байта) + 3 таймера полива ( water0run ,
  water1run ,  water2run  по 1 байту) + 1 состояние заслонки ( timerFlap  1 байт).
  • Стало: 4 уставки температуры (8 байт) + 1 таймер полива ( water2run  1 байт) + 3 состояния заслонки ( flapMin ,
  flapMax ,  flapCurr  по 1 байту).
